...
#' Add a scalebar with label to an image panel
#'
#' Overlays a horizontal scalebar and corresponding text label onto an
#' existing `ggdraw`/`ggplot` object. The scalebar is positioned relative
#' to the image region (as defined by `img_fraction`) and can be anchored
#' to either the bottom-left or bottom-right corner.
#'
#' A semi-transparent background box is drawn behind the scalebar and label
#' to ensure readability against varying image backgrounds.
#'
#' @param plot A `ggdraw` or `ggplot` object to which the scalebar is added.
#' @param img_fraction Numeric between 0 and 1. Fraction of total panel height
#'        occupied by the image (used to align the scalebar within the image region).
#' @param label_gap Numeric. Vertical gap between label and image used in the
#'        parent layout. Must match the value used in `add_top_label()`.
#' @param bar_width Numeric. Relative width of the scalebar as a fraction of
#'        the image width.
#' @param bar_height Numeric. Thickness of the scalebar (currently not used
#'        explicitly for rendering but retained for future extension).
#' @param x_offset Numeric. Horizontal offset from the anchor side, expressed
#'        as a fraction of image width.
#' @param y_offset Numeric. Vertical offset from the bottom of the image,
#'        expressed as a fraction of image height.
#' @param label Character string. Text displayed above the scalebar.
#' @param text_size Numeric. Font size of the scalebar label.
#' @param text_offset Numeric. Vertical spacing between scalebar and label,
#'        relative to image height.
#' @param color Character string. Color of scalebar and label text.
#' @param bg_fill Character string. Fill color of the background box.
#' @param bg_alpha Numeric between 0 and 1. Transparency of the background box.
#' @param bg_padding Numeric. Padding around scalebar and label inside the
#'        background box, expressed in normalized panel coordinates.
#' @param anchor Character string. Horizontal anchoring of the scalebar.
#'        Must be either `"left"` or `"right"`.
#'
#' @return A modified `ggdraw` object with scalebar and label overlay.
#'
#' @details The scalebar is drawn in normalized panel coordinates but restricted
#' to the image region using `img_fraction`. Text height is approximated to
#' construct the background box, ensuring consistent appearance across different
#' output sizes.
#'
#' The function assumes a square image region aligned to the bottom-left of the
#' panel, consistent with the layout generated by `add_top_label()`.
#'
#' @importFrom cowplot draw_grob draw_line draw_label
